 ...
 */
void simulate(line_t *from, line_t *to, int lines)
{
...
int main(int argc, char** argv)
{
	...
	ca_init(argc, argv, &lines, &its);
        ...
        line_t *from, *to, *from_d, *to_d;

	from = (line_t *) calloc((lines + 2), sizeof(line_t));
	to = (line_t *) calloc((lines + 2), sizeof(line_t));

	ca_init_config(from, lines, 0);

        // cudaMalloc((void **) &from)
